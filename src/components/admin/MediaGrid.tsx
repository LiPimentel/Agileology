...
export type MediaItem = { id: string; url: string; altText: string | null; mimeType?: string };
          ...
          <div key={item.id} className="group relative overflow-hidden rounded-md border border-slate-200">
            ...
            {!onSelect && (
              <button
                type="button"
                onClick={() => {
                  if (confirm(isVideo ? "¿Eliminar este video?" : "¿Eliminar esta imagen?")) deleteMedia(item.id);
                }}
